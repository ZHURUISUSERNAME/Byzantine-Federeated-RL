...
class Environment:
    ...
    def step(self, action):
        next_state, reward_original,c1_,c2_,c3_ = self.get_next_state(self.day_number, self.time_step, self.current_state,action)
        self.time_step += 1
    ...
    def get_next_state(self, day_number, time_step, state_k, action_k):
        ...
        if action_k[0] >= 0:
            p_charge, p_discharge = np.clip(action_k[0],0,min((self.env_options.E_max-current_energy)/self.eta,self.env_options.P_cap)), 0.0
            if p_charge==0:
                action_k[0]=0
        else:
            p_charge, p_discharge = 0.0, np.clip(action_k[0],max(-self.env_options.P_cap,self.eta*(self.env_options.E_min-current_energy)),0)
            if p_discharge==0:
                action_k[0]=0
        ...
        T_next = self.env_options.Ewuxilong*current_indoorTemp+(1-self.env_options.Ewuxilong)*(current_outdoorTemp-self.env_options.eta_hvac*action_k[1]/self.env_options.A)
        ...
        next_state = [self.get_solar(day_number, time_step + 1), self.get_load(day_number, time_step + 1), e_next,self.get_outdoorTemp(day_number, time_step + 1),T_next,
